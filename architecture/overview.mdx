...
Osintly runs full searches through an API-driven pipeline with progressive delivery to the result page. Limited [free previews](/guides/free-searches) follow a separate, temporary access flow and are not saved to account history.
...
<Steps>
  ...
  <Step title="Authorize execution">
    Plan entitlements, selected workspace access, and the applicable personal or organization credit source are validated.
  </Step>
...
<AccordionGroup>
  <Accordion title="What consumes credits" icon="coins">
    Launching a new full app search consumes credits. Free previews use a separate quota.
  </Accordion>
