...
<AccordionGroup>
  ...
  <Accordion title="Controlled access" icon="user-lock">
    Use organization roles (owner/admin/member) to limit sensitive actions to authorized users. Keep Personal and organization workspaces separate when handling a sensitive case.
  </Accordion>
...
<Tabs>
  <Tab title="What Osintly provides">
    Search launch and streaming UI, result history, legacy Monitoring transition guidance, collaboration features, and export options exposed by the interface.
  </Tab>
...
<Note>
For current plan features and collaboration limits, refer to [osint.ly/pricing](https://osint.ly/pricing). See [Monitoring retirement](/operations/monitoring) for the legacy feature’s status.
</Note>
