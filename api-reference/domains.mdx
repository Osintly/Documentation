...
<Warning>
`Domain Name` and `IP Address` remain valid Search API types for now, but they will soon stop being supported there. For new lookups, use their replacements: [Domain Intelligence](/tools-reference/domain-intelligence) and [IP Intelligence](/tools-reference/ip-intelligence). No cutoff date has been announced.
</Warning>
...
Fetch current module IDs from `GET /modules` for the selected search type. The UUIDs below illustrate the request shape; do not copy them into production requests without checking the live catalog.
