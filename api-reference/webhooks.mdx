...
<Tip>
If you need to re-fetch full data, call `GET /search/{id}`, `GET /search/{id}/results`, and `GET /search/{id}/results/leaks` using the `searchId` from the webhook payload.
</Tip>
