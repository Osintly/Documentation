...
The webhook payload contains the aggregated stored result payload, while `GET /search/{id}/results` and `GET /search/{id}/results/leaks` expose the normalized API shapes for retrieval.
