...
This page documents how Osintly API plans work, how they differ from Osintly app plans, how API access follows the active workspace, and how runtime capacity is enforced.
...
- An organization can hold its own API plan. It remains separate from the organization app plan and from each member's personal API plan.

## API workspace scope

Open [https://osint.ly/workspace/api](https://osint.ly/workspace/api) after selecting the workspace you want to manage.

| Active workspace | API account used |
|---|---|
| Personal | Your personal API keys, usage, and billing |
| Organization | The organization’s API keys, usage, and billing |

Within an organization, owners and admins can create, revoke, and manage API keys. All members can view API keys and usage. Only the owner can manage organization API billing and invoices.

An organization API key belongs to the organization, not to the member who created it. Switching workspaces does not move keys, usage, subscriptions, or invoices.
...
- The website's limited [free search previews](/guides/free-searches) are a separate feature. They do not grant `POST /search` access on the Free API plan.
...
For an organization, API billing remains separate from the organization app subscription and is managed by the organization owner in the active organization workspace.
